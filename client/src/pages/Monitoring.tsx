    ...
    () => [
      {
        ...
        percent:
          data && data.messageStats.totalMessages > 0
        ...
        >
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div className="space-y-2">
              ...
              <div>
                ...
                <p className="mt-2 max-w-3xl text-sm text-[var(--aston-muted)]">
                  메시지 분포, 응답 지연, 시스템 상태를 한 화면에서 확인합니다.
                </p>
              </div>
            </div>
            ...
            <Button
              ...
              onClick={() => refetch()}
              disabled={isFetching}
              className="border-white/10 bg-white/5 text-[var(--aston-text)] hover:border-cyan-500/30 hover:bg-white/10"
            >
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {Array.from({ length: 4 }).map((_, index) => (
                <div
                  key={index}
                  className="h-28 animate-pulse rounded-2xl border border-white/10 bg-black/15"
                />
              ))}
            </div>
            <div className="h-80 animate-pulse rounded-2xl border border-white/10 bg-black/15" />
            <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
              <div className="h-72 animate-pulse rounded-2xl border border-white/10 bg-black/15" />
              <div className="h-72 animate-pulse rounded-2xl border border-white/10 bg-black/15" />
            </div>
          </div>
        ) : data ? (
          <>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <MetricCard
                ...
                detail={`${formatNumber(data.performance.responseCount)}회 측정`}
              />
              <MetricCard
                icon={Radar}
                label="활성 대화"
                value={formatNumber(data.conversationStats.activeConversations)}
                detail={`${formatNumber(data.conversationStats.pinnedConversations)}개 고정`}
              />
              <MetricCard
                icon={Server}
                label="시스템 업타임"
            ...
            <section className="rounded-2xl border border-white/10 bg-[var(--aston-panel)] p-4 md:p-6">
              <div className="mb-4 flex items-center gap-2">
                <Brain className="h-4 w-4 text-cyan-400" />
                <h2 className="text-lg font-semibold text-[var(--aston-text)]">API 사용량</h2>
              </div>
              ...
              <div className="mt-5">
                <h3 className="mb-3 text-sm font-semibold text-[var(--aston-text)]">최근 활동 로그</h3>
                <div className="space-y-2">
                  {data.apiUsage.activityLogs.length > 0 ? (
                    data.apiUsage.activityLogs.map((entry) => (
                      <div
                        key={`${entry.at}-${entry.engine}-${entry.model}`}
                        className="flex flex-col gap-1 rounded-xl border border-white/10 bg-black/15 px-4 py-3 md:flex-row md:items-center md:justify-between"
                      >
                        <div>
                          <div className="text-sm font-medium text-[var(--aston-text)]">
                        ...
                        <div className="text-xs text-[var(--aston-muted)]">
                          ...
                          {entry.tokensUsed !== null ? ` · ${formatNumber(entry.tokensUsed)} tokens` : ""}
                        </div>
                      </div>
                    ))
                  ) : (
                    <div className="rounded-xl border border-white/10 bg-black/15 px-4 py-3 text-sm text-[var(--aston-muted)]">
                      아직 기록된 활동이 없습니다.
                    </div>
                  )}
                </div>
            ...
            <div className="grid gap-4 xl:grid-cols-[1.4fr_0.9fr]">
              <section className="rounded-2xl border border-white/10 bg-[var(--aston-panel)] p-4 md:p-6">
                <div className="mb-4">
                  <h2 className="text-lg font-semibold text-[var(--aston-text)]">7일 메시지 추이</h2>
                  <p className="text-sm text-[var(--aston-muted)]">전체, 사용자, 어시스턴트 메시지 분포를 비교합니다.</p>
                </div>

                <ChartContainer config={trendChartConfig} className="h-80 w-full aspect-auto">
                  <LineChart data={trendData} margin={{ left: 8, right: 16, top: 8, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="messages" stroke="var(--color-messages)" strokeWidth={3} dot={false} />
                    <Line type="monotone" dataKey="userMessages" stroke="var(--color-userMessages)" strokeWidth={2} dot={false} />
                    <Line type="monotone" dataKey="assistantMessages" stroke="var(--color-assistantMessages)" strokeWidth={2} dot={false} />
                  </LineChart>
                </ChartContainer>
              </section>
              ...
              <section className="rounded-2xl border border-white/10 bg-[var(--aston-panel)] p-4 md:p-6">
                <div className="mb-4">
                  <h2 className="text-lg font-semibold text-[var(--aston-text)]">메시지 소스 분포</h2>
                  <p className="text-sm text-[var(--aston-muted)]">웹과 Telegram의 유입 비율을 확인합니다.</p>
                </div>
                  ...
                  {sourceData.map((item, index) => (
                    <div key={item.key} className="space-y-2">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-[var(--aston-muted)]">{item.label}</span>
                        <span className="font-medium text-[var(--aston-text)]">
                ...
                <div className="mt-6 grid gap-3 sm:grid-cols-2">
                  <InfoPill label="총 대화 수" value={formatNumber(data.conversationStats.totalConversations)} />
                  <InfoPill label="고정 대화" value={formatNumber(data.conversationStats.pinnedConversations)} />
                  <InfoPill label="웹 메시지" value={formatNumber(data.messageStats.webMessages)} />
                  <InfoPill label="Telegram 메시지" value={formatNumber(data.messageStats.telegramMessages)} />
                </div>
              </section>
            </div>
            ...
            <div className="grid gap-4 xl:grid-cols-[1fr_1fr]">
              <section className="rounded-2xl border border-white/10 bg-[var(--aston-panel)] p-4 md:p-6">
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold text-[var(--aston-text)]">응답 성능</h2>
                </div>
              ...
              <section className="rounded-2xl border border-white/10 bg-[var(--aston-panel)] p-4 md:p-6">
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold text-[var(--aston-text)]">시스템 상태</h2>
                </div>
...
function MetricCard({
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[var(--aston-panel)] p-4 md:p-5">
      <div className="mb-3 flex items-center gap-2 text-sm text-[var(--aston-muted)]">
        <Icon className="h-4 w-4 text-cyan-400" />
        <span>{label}</span>
      </div>
      <div className="text-2xl font-semibold text-[var(--aston-text)]">{value}</div>
      <div className="mt-1 text-sm text-[var(--aston-muted)]">{detail}</div>
    </div>
  );
}
...
function InfoPill({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-white/10 bg-black/15 px-4 py-3">
      <div className="text-xs uppercase tracking-wide text-[var(--aston-muted)]">{label}</div>
      <div className="mt-1 break-all text-sm font-medium text-[var(--aston-text)]">{value}</div>
    </div>
  );
}
