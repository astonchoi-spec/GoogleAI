...
반드시 JSON만 응답: { intent: string, params: object, clarification: string|null }
파라미터 부족 시 clarification에 질문 담아서 반환.`;
...
export async function executeIntent(parsed: ParsedIntent, ctx: TrpcContext): Promise<string> {
  ...
  switch (parsed.intent) {
    ...
    default:
      return "처리할 수 있는 기능을 찾지 못했습니다. 일반 질문으로 다시 입력해주세요.";
  }
}
 ...
 * }
 *
 * 일반 대화는 기존 LLM 호출 흐름을 그대로 유지하면 됩니다.
 */
...
function requiredExchange(value: unknown): SupportedExchangeId {
  const exchange = requiredString(value, "거래소를 알려주세요. 가능한 값: binance, upbit, bybit");
...
function requiredSide(value: unknown): FuturesRiskInput["side"] {
  const side = requiredString(value, "포지션 방향을 알려주세요. 가능한 값: long, short").toLowerCase();
...
function requiredAlertType(value: unknown): "price" | "rsi" | "funding" | "kimchi_premium" {
  const type = requiredString(value, "알림 유형을 알려주세요. 가능한 값: price, rsi, funding, kimchi_premium");
  if (["price", "rsi", "funding", "kimchi_premium"].includes(type)) {
    return type as "price" | "rsi" | "funding" | "kimchi_premium";
  }
  throw new Error("지원 알림 유형은 price, rsi, funding, kimchi_premium입니다.");
}

function requiredAlertOperator(value: unknown): "above" | "below" {
  const operator = requiredString(value, "알림 조건을 알려주세요. 가능한 값: above, below");
...
function requiredTradeStatsPeriod(value: unknown): TradeStatsPeriod {
  const period = requiredString(value, "통계 기간을 알려주세요. 가능한 값: week, month");
...
function requiredNewDeal(params: Record<string, unknown>): NewPFDeal {
  return {
    ...
    loanAmount: requiredNumber(params.loanAmount, "대출금을 알려주세요."),
