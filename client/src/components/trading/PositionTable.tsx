...
export default function PositionTable() {
  ...
  // Binance API 키 없음 — 한 줄 안내로 대체
  if (!positionsQuery.isLoading && positionsQuery.isError) {
    return (
      <div className="rounded-2xl border border-white/10 bg-[var(--aston-panel)] flex items-center gap-2 px-4 py-3 text-sm text-[var(--aston-muted)]">
        <span className="font-medium text-[var(--aston-text)]">Open Positions</span>
        <span>— API 키를 설정하면 잔고/포지션을 확인할 수 있습니다</span>
      </div>
    );
  }
